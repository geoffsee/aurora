...
export function stepAudioEma(
  ...
  state.bass = step(state.bass, raw.bass, alphas.bass, releaseAlphas?.bass ?? alphas.bass);
  state.mid = step(state.mid, raw.mid, alphas.mid, releaseAlphas?.mid ?? alphas.mid);
  state.high = step(state.high, raw.high, alphas.high, releaseAlphas?.high ?? alphas.high);
  state.pulse = step(state.pulse, raw.pulse, alphas.pulse, releaseAlphas?.pulse ?? alphas.pulse);
  return state;
}
