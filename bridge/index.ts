...
import { makeAudioControlRouter, parseAudioMappings } from './audio-control-router.ts';
import {
  type AudioEmaAlphas,
  type AudioFeatures,
...
const osc = require('osc') as {
  UDPPort: new (
    ...
    on: {
      ...
      (event: 'message', cb: (msg: OscMsg) => void): void;
...
// Bind both HTTP servers on all interfaces so LAN clients (phone, second
...
const stateLogCapacity = Number.isFinite(_raw) && _raw >= 1 ? Math.floor(_raw) : 500;
...
const resolveStaticFile = (relativePath: string) => {
  const base =
    relativePath.startsWith('dist/') || relativePath.startsWith('assets/') ? root : webRoot;
...
const oscArgValue = (arg: OscArg) =>
  arg && typeof arg === 'object' && 'value' in arg ? (arg as { value: unknown }).value : arg;
...
// without a preset behind it, and vice versa.
{
  ...
  for (const name of expected) {
    if (!cueNames.has(name)) {
      throw new Error(`cuePresets missing cue "${name}" required by vst-osc-contract.json`);
    }
  }
  for (const name of cueNames) {
    if (!expected.has(name)) {
      throw new Error(`cuePresets has cue "${name}" not declared in vst-osc-contract.json`);
    }
  }
}
const coerceControlState = (state: unknown): ControlState => {
  ...
  return {
    ...
    flashVersion: clampInt(source.flashVersion, 0, Number.MAX_SAFE_INTEGER, defaults.flashVersion),
    resetVersion: clampInt(source.resetVersion, 0, Number.MAX_SAFE_INTEGER, defaults.resetVersion),
    cueVersion: clampInt(source.cueVersion, 0, Number.MAX_SAFE_INTEGER, defaults.cueVersion),
    ...
    cueDeckAMode: clampInt(source.cueDeckAMode, 0, 48, defaults.cueDeckAMode),
    cueDeckBMode: clampInt(source.cueDeckBMode, 0, 48, defaults.cueDeckBMode),
    ...
    layerWeight0: clamp(source.layerWeight0, 0, 1, latestControlState?.layerWeight0 ?? 0),
    layerWeight1: clamp(source.layerWeight1, 0, 1, latestControlState?.layerWeight1 ?? 0),
    layerWeight2: clamp(source.layerWeight2, 0, 1, latestControlState?.layerWeight2 ?? 0),
    layerWeight3: clamp(source.layerWeight3, 0, 1, latestControlState?.layerWeight3 ?? 0),
    layerWeight4: clamp(source.layerWeight4, 0, 1, latestControlState?.layerWeight4 ?? 0),
    layerWeight5: clamp(source.layerWeight5, 0, 1, latestControlState?.layerWeight5 ?? 0),
    layerWeight6: clamp(source.layerWeight6, 0, 1, latestControlState?.layerWeight6 ?? 0),
    layerWeight7: clamp(source.layerWeight7, 0, 1, latestControlState?.layerWeight7 ?? 0),
    figureModel: clampInt(source.figureModel, 0, MAX_FIGURE_MODEL_INDEX, defaults.figureModel),
...
const maybeFeedAutomationAudio = (features: AudioFeatures, nowMs: number): void => {
...
function applyTransientConfigMsg(address: string, firstArg: unknown): void {
  ...
  switch (key) {
    ...
    case 'debounce':
      automationBridge.updateTransientConfig({
        debounceMs: clamp(firstArg, 0, 60000, DEFAULT_TRANSIENT_CONFIG.debounceMs),
      });
      break;
    case 'mode': {
...
const broadcast = (msg: OscMsg) => {
  ...
  if (msg.address === OSC_ADDRESSES.NUM_TRACKS) {
    const n = oscArgValue(msg.args?.[0]);
...
const broadcastControl = (state: unknown) => {
  ...
  if (prev && prev.audioTransientAutomation !== latestControlState.audioTransientAutomation) {
    automationBridge.resetTransientDetector(
      latestControlState.audioTransientAutomation ? latestAutomationAudioFeatures : undefined,
    );
  }
...
const numericArg = (arg: OscArg | undefined, fallback: number) =>
  finiteNumber(oscArgValue(arg), fallback);
const applyVstControlMessage = (msg: OscMsg) => {
  ...
  if (msg.address.startsWith(VST_CUE_PREFIX)) {
    ...
    mergeControlState({
      ...
      flashVersion: name === 'panic' ? current.flashVersion : current.flashVersion + 1,
    });
  }
};
...
const applyPresetMorph = (msg: OscMsg) => {
  const args = msg.args ?? [];
  const fromName = String(oscArgValue(args[0]));
  const toName = String(oscArgValue(args[1]));
  const position = clampMorphPosition(oscArgValue(args[2]));
  const curveArg = args.length > 3 ? oscArgValue(args[3]) : undefined;
...
const layerController = createLayerController({
  ...
  onFull: () =>
    console.error(`[OSC] dropping preset layer add — stack already at max ${PRESET_LAYER_MAX}`),
});

const applyPresetLayer = (msg: OscMsg) => {
  const args = msg.args ?? [];
  const intArg = (i: number) => Math.trunc(Number(oscArgValue(args[i])));

  switch (msg.address) {
    case PRESET_LAYER_ADD_ADDRESS: {
      const name = String(oscArgValue(args[0]));
      const weight = args.length > 1 ? Number(oscArgValue(args[1])) : 1;
      layerController.add({
        name,
    ...
    case PRESET_LAYER_WEIGHT_ADDRESS:
      layerController.setWeight(intArg(0), Number(oscArgValue(args[1])));
      break;
...
const isAbletonLinkActive = (): boolean => isLinkActive(lastLinkUpdateAt, Date.now());
...
type AbletonLinkSession = {
  ...
  quantum?: number;
  numPeers?: number;
  startUpdate: (intervalMs: number, cb: (beat: number, phase: number, bpm: number) => void) => void;
};
...
function broadcastLinkTempo(tempo: number): void {
  ...
  if (Math.abs(rounded - lastLinkTempoValue) < 0.05 && Date.now() - lastLinkTempoSent < 200) {
    return;
  }
...
function startAbletonLink(): void {
  ...
  const quantum =
    typeof link.quantum === 'number' && link.quantum > 0 ? link.quantum : LINK_DEFAULT_QUANTUM;
  ...
  link.startUpdate(LINK_UPDATE_INTERVAL_MS, (beat, phase, bpm) => {
    ...
    const frame = deriveLinkFrame({ beat, phase, bpm, numPeers: linkNumPeers }, quantum);
    if (!frame) return;
...
const visualServer = Bun.serve({
  ...
  async fetch(request, server) {
    ...
    return new Response(file, {
      headers: {
        ...
        'cache-control': relativePath.startsWith('dist/pkg/') ? 'no-store' : 'public, max-age=30',
  ...
  websocket: {
    ...
    message(ws, raw) {
      try {
        const parsed = JSON.parse(raw.toString()) as Partial<OscMsg> & Record<string, unknown>;
        if (typeof parsed.address === 'string') {
          if (parsed.address === '/aurora/control/state') {
          ...
          } else if (parsed.address === '/aurora/error' && typeof parsed.error === 'string') {
            ...
            if (validatePresetOscMsg({ address: parsed.address }, 'WS client')) {
            ...
            sendOsc(parsed.address, Array.isArray(parsed.args) ? parsed.args : []);
          }
        }
      } catch (error) {
        console.error('bad websocket OSC message', error instanceof Error ? error.message : error);
      }
    },
  },
});
...
const getShadertoyKey = (): string => runtimeShadertoyKey ?? Bun.env.SHADERTOY_API_KEY ?? '';
  ...
  async fetch(request) {
    ...
    if (pathname === '/api/shadertoy/key') {
      ...
      if (request.method === 'POST') {
        ...
        if (!key) {
          return Response.json({ ok: false, error: 'Missing `key` field' }, { status: 400 });
        }
        if (!SHADERTOY_KEY_RE.test(key)) {
          return Response.json(
            {
              ok: false,
              error: 'Key must be 8–128 alphanumeric characters (Shadertoy API key format)',
            },
            { status: 400 },
          );
        }
    ...
    if (request.method === 'POST' && pathname === '/api/shadertoy/import') {
      ...
      if (typeof target !== 'string' || target.length === 0) {
        return Response.json({ ok: false, error: 'Missing `url` or `id` field' }, { status: 400 });
      }
...
udp.on('ready', () => {
  oscReady = true;
  console.log(`OSC ready: listening :${liveRecvPort}, sending to ${liveHost}:${liveSendPort}`);
...
vstControlUdp.on('message', (msg: OscMsg) => {
  if (msg.address === PRESET_MORPH_ADDRESS) {
    if (validatePresetMorphOscMsg(msg, `VST :${vstControlRecvPort}`, cueNames)) {
      applyPresetMorph(msg);
    }
    return;
  }
  if (msg.address.startsWith(PRESET_LAYER_PREFIX)) {
    if (validatePresetLayerOscMsg(msg, `VST :${vstControlRecvPort}`, cueNames)) {
  ...
  if (msg.address.startsWith('/aurora/automation/transient/')) {
    applyTransientConfigMsg(msg.address, oscArgValue(msg.args?.[0]));
    return;
  }
...
setInterval(() => {
  ...
  const energy = 0.45 + Math.sin(now * 2.1) * 0.2 + Math.max(0, Math.sin(now * 8.0)) * 0.25;
  const rawFeatures: AudioFeatures = {
