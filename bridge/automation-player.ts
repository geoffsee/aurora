...
// automation replay (including audio triggers) must not override them.
...
export function filterAutomationDiff(diff: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function makeAutomationPlayer(mergeControlState: (diff: Record<string, unknown>) => void): {
  ...
  function tick() {
    ...
    while (cursor < recording.frames.length) {
      ...
      if (!frame || frame.tMs > elapsed) break;
