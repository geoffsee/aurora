...
const osc = require("osc") as {
  UDPPort: new (opts: Record<string, unknown>) => {
    send: (msg: OscMsg) => void;
    open: () => void;
    on: (event: "ready" | "message" | "error", cb: (...args: any[]) => void) => void;
  };
};
...
function contentType(pathname: string) {
  ...
  return dot === -1 ? "application/octet-stream" : mimeTypes[pathname.slice(dot)] ?? "application/octet-stream";
}
...
const cuePresets: Record<string, Partial<ControlState>> = {
  ...
  tunnel: { crossfade: 0.25, intensity: 1, feedback: 0.5, depth: 0.85, palette: 0.66, deckAMode: 1, deckBMode: 3 },
...
const coerceControlState = (state: unknown): ControlState => {
  ...
  const mapping = source.trackMapping && typeof source.trackMapping === "object" ? source.trackMapping : {};
  ...
  return {
    ...
    trackMapping: {
      deckAStart: clampInt((mapping as Partial<TrackMapping>).deckAStart, 0, 31, defaults.trackMapping.deckAStart),
      deckACount: clampInt((mapping as Partial<TrackMapping>).deckACount, 1, 32, defaults.trackMapping.deckACount),
      deckBStart: clampInt((mapping as Partial<TrackMapping>).deckBStart, 0, 31, defaults.trackMapping.deckBStart),
      deckBCount: clampInt((mapping as Partial<TrackMapping>).deckBCount, 1, 32, defaults.trackMapping.deckBCount),
      bassTrack: clampInt((mapping as Partial<TrackMapping>).bassTrack, 0, 31, defaults.trackMapping.bassTrack),
      midTrack: clampInt((mapping as Partial<TrackMapping>).midTrack, 0, 31, defaults.trackMapping.midTrack),
      highTrack: clampInt((mapping as Partial<TrackMapping>).highTrack, 0, 31, defaults.trackMapping.highTrack),
    },
  };
};
...
const numericArg = (arg: OscArg | undefined, fallback: number) => finiteNumber(valueOf(arg), fallback);
  ...
  if (msg.address.startsWith(controlPrefix)) {
    ...
    switch (name) {
      ...
      case "beat_sync":
        ...
        break;
      case "bar_sync":
        mergeControlState({ barSync: booleanArg(arg), beatSync: current.beatSync || booleanArg(arg) });
        break;
      case "demo_mode":
...
const visualServer = Bun.serve({
  ...
  websocket: {
    open(ws) {
      sockets.add(ws);
      ws.send(JSON.stringify({ address: "/bevyosc/osc/connected", args: [oscReady ? 1 : 0] }));
      if (latestControlState) {
        ws.send(JSON.stringify({ address: "/bevyosc/control/state", args: [latestControlState] }));
      }
    },
    close(ws) {
      sockets.delete(ws);
      ...
      try {
        ...
        if (typeof parsed.address === "string") {
          if (parsed.address === "/bevyosc/control/state") {
            broadcastControl(Array.isArray(parsed.args) ? parsed.args[0] : null);
          } else {
...
vstControlUdp.on("error", (error: Error) => console.error("VST control OSC error:", error.message));
vstControlUdp.open();
...
setInterval(() => {
  ...
  const data = JSON.stringify({ address: "/bevyosc/server/diagnostics", args: [diagnostics] });
  sockets.forEach((ws) => ws.send(data));
}, 500);
