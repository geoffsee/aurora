 ...
 */
export const aurora_crownVariant = paletteVariantShell((uv, time, hue_shift, pulse, energy, bass, mid, high) => {
  'use gpu';
  ...
  let flow = fbm(vec2f(p.x * 0.35 - time * 0.022, p.y * 0.9 + time * 0.03));
  let flow2 = fbm(vec2f(p.x * 1.1 + flow * 0.6, p.y * 1.6 - time * 0.045));

  // Parabolic arcs / crown bands across the sky.
  let arc_y = p.y + 0.15 * std.cos(p.x * 1.1 + time * 0.18 + flow * 1.4)
    + 0.06 * std.sin(p.x * 2.8 - time * 0.31 + flow2);
  let band_phase = (arc_y + flow * 0.18) * (3.4 + mid * 2.6) + time * (0.12 + bass * 0.2);
  let bands = std.pow(0.5 + 0.5 * std.cos(band_phase * TAU), 2.0 + high * 1.4);

  // Second set of thinner high-altitude ribbons.
  let ribbon_phase = (arc_y * 1.45 - p.x * 0.08 + flow2 * 0.25) * (5.2 + high * 3.0) - time * 0.2;
  let ribbons = std.pow(0.5 + 0.5 * std.cos(ribbon_phase * TAU), 3.2 + pulse * 1.5);

  // Soft coronal glow rising from horizon center.
  let crown_r = std.length(vec2f(p.x * 0.55, std.max(p.y + 0.35, 0.0) * 1.15));
  let corona = std.exp(-crown_r * (2.4 - bass * 0.8 - energy * 0.5))
    * (0.35 + bass * 0.45 + pulse * 0.25)
    * std.smoothstep(-0.9, 0.2, p.y);

  // Upward ray spikes on pulse (magnetic field lines).
  let ray_a = std.atan2(p.y + 0.4, p.x + 0.0001);
  let spike = std.pow(std.max(0.0, std.cos(ray_a * (7.0 + std.floor(mid * 5.0)) + time * 0.4)), 10.0)
    * std.smoothstep(0.0, 0.85, p.y + 0.35)
    * (0.08 + pulse * 0.55 + high * 0.2)
    * (1.0 - std.smoothstep(0.6, 1.5, crown_r));

  // Horizon band window.
  let sky = std.smoothstep(-1.05, 0.05, p.y) * (1.0 - std.smoothstep(0.65, 1.4, p.y));
  ...
  let body = std.mix(vec3f(0.15, 0.55, 0.62), pick, 0.35);
  let tip = std.mix(vec3f(0.78, 0.28, 0.85), pick, 0.22);
  let rose = vec3f(0.92, 0.42, 0.62);
  ...
  let color = std.mix(base * 0.5, accent, std.clamp(bands * 0.7 + ribbons * 0.55 + corona * 0.4, 0.0, 1.0));
  ...
  let star_cell = std.floor(p * vec2f(36.0, 24.0) + vec2f(time * 0.02, 0.0));
  let star_h = hash21(star_cell);
  let star_local = std.fract(p * vec2f(36.0, 24.0) + vec2f(time * 0.02, 0.0)) - 0.5;
  let tw = 0.55 + 0.45 * std.sin(time * (2.5 + star_h * 6.0) + star_h * 20.0);
  let star = std.step(0.93 - high * 0.04, star_h)
  ...
  let alpha = std.clamp(layer * 0.9 + star * 0.55, 0.0, 1.0) * enabled;
