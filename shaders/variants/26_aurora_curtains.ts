 ...
 */
export const aurora_curtainsVariant = paletteVariantShell(
  (uv, time, hue_shift, pulse, energy, bass, mid, high) => {
    ...
    const wind = fbm(vec2f(p.x * 0.55 + time * 0.028, p.y * 0.22 - time * 0.018));
    const wind2 = fbm(vec2f(p.x * 1.35 - time * 0.04, p.y * 0.55 + wind * 0.8 + time * 0.012));
    const wind3 = noise(vec2f(p.x * 3.2 + wind2 * 1.4, p.y * 1.1 - time * 0.07));

    // Vertical fold displacement (classic curtain undulation).
    const fold =
      std.sin(p.y * (1.65 + mid * 2.4) + time * (0.22 + high * 0.45) + wind * 3.1) *
    ...
    const phase1 = (p.x + fold + wind * 0.32) * (1.9 + mid * 1.6);
    const phase2 = (p.x * 1.35 + fold * 0.7 - wind2 * 0.28 + 0.37) * (2.8 + bass * 1.4);
    const phase3 = (p.x * 0.72 + fold * 1.2 + wind3 * 0.18 - 0.21) * (3.6 + high * 2.2);

    const sheet1 = std.pow(0.5 + 0.5 * std.cos(phase1 * TAU), 1.9 + high * 1.1);
    const sheet2 = std.pow(0.5 + 0.5 * std.cos(phase2 * TAU), 2.4 + mid * 1.2);
    const sheet3 = std.pow(0.5 + 0.5 * std.cos(phase3 * TAU), 3.0 + high * 1.8);

    // Bright fringes along sheet edges.
    const edge1 =
      1.0 -
      std.smoothstep(0.0, 0.18 + high * 0.1, std.abs(std.fract(phase1 + wind * 0.12) - 0.5) * 2.0);
    const edge2 =
      1.0 - std.smoothstep(0.0, 0.14 + pulse * 0.08, std.abs(std.fract(phase2 + 0.22) - 0.5) * 2.0);

    // Vertical silk grain (raylets along the curtain).
    const silk = fbm(vec2f(phase1 * 0.55 + wind2, p.y * 3.4 - time * 0.08));
    const rays =
      std.pow(
        std.max(
          0.0,
          1.0 - std.abs(std.sin(p.y * (18.0 + high * 22.0) + phase1 * 2.0 + time * 0.9)),
        ),
        6.0,
      ) *
    ...
    const sky = std.smoothstep(-1.15, -0.05, p.y) * (1.0 - std.smoothstep(0.55, 1.35, p.y));
    const ground = std.smoothstep(-1.2, -0.55, p.y) * 0.08;

    // Ambient floor so idle (no audio) still glows; audio lifts brightness.
    const ambient = 0.55 + energy * 0.55 + pulse * 0.18;
    const sheets = sheet1 * 0.55 + sheet2 * 0.38 + sheet3 * 0.28;
    const layer =
      (sheets * ambient +
        edge1 * (0.14 + high * 0.28) +
        edge2 * (0.1 + mid * 0.18) +
        silk * 0.14 +
        rays +
        ground) *
    ...
    const aurora_green = vec3f(0.12, 0.78, 0.42);
    const aurora_cyan = vec3f(0.18, 0.92, 0.72);
    const pick = vec3f(palette_rgb.x, palette_rgb.y, palette_rgb.z);
    const base_tint = std.mix(aurora_green, pick, 0.42);
    const edge_tint = std.mix(aurora_cyan, pick, 0.28);

    const hue_flow = hue_shift * 0.12 + phase1 * 0.05 + wind * 0.35 + time * 0.008;
    const base = vjDuotone(base_tint, hue_flow, 0.82 * sat, bri);
    const accent = vjDuotone(edge_tint, hue_flow + 0.08 + p.y * 0.12, sat, bri * 1.05);
    const tip = vjDuotone(vec3f(0.55, 0.95, 0.62), hue_flow + edge1 * 0.15, 0.7 * sat, bri);
    ...
    const star_cell = std.floor(p * vec2f(42.0, 28.0));
    const star_h = hash21(star_cell);
    const star_local = std.fract(p * vec2f(42.0, 28.0)) - 0.5;
    const star =
      std.step(0.97, star_h) *
    ...
    const alpha = std.clamp(layer * 0.92 + star * 0.4, 0.0, 1.0) * enabled;
