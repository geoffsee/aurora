...
export const recursive_mawVariant = paletteVariantShell(
  (uv, time, _hue_shift, pulse, energy, bass, mid, high) => {
    ...
    const r0 = std.length(p);
    let acc = 0.0;
    let wsum = 0.0;
    const cbase =
      vec2f(-0.72, 0.18) + vec2f(std.sin(time * 0.1) * 0.06, std.cos(time * 0.13) * 0.05);
    const cmod = vec2f(high * 0.18, mid * 0.12) * std.sin(time * 0.7);
    const c = cbase + cmod;

    for (let i = 0; i < 6; i++) {
      const z = p * (1.6 + bass * 0.6);
      const z2 = std.dot(z, z);
      const inv = 1.0 / std.max(z2, 0.02);
      p = vec2f(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y) * inv + c;
      const contrib = std.exp(-z2 * (0.7 + f32(i) * 0.15)) * (0.8 + 0.4 * pulse);
      acc += contrib;
      wsum += 1.0;
    }
    const field = std.clamp(acc / wsum, 0.0, 2.0);

    const depth = 1.0 / std.max(r0, 0.001) + time * (1.2 + bass * 2.0);
    const ring =
      1.0 - std.smoothstep(0.0, 0.14 + bass * 0.1, std.abs(std.fract(depth) - 0.5) * 2.0);
    const spokes =
      1.0 -
      std.smoothstep(
        0.0,
        0.05 + high * 0.08,
        std.abs(std.fract((std.atan2(p.y, p.x) / TAU) * (6.0 + mid * 8.0) + time * 0.3) - 0.5) *
          2.0,
      );

    const sat = std.clamp(palette_extra.x, 0.0, 1.0);
    const bri = std.clamp(palette_extra.y, 0.0, 1.0);
    const hue_phase = std.atan2(p.y, p.x) * 0.3 + depth * 0.03 + field * 0.4;
    ...
    const layer = field * 0.9 + ring * (0.6 + bass * 0.6) + spokes * (0.35 + high * 0.5);
