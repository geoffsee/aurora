...
export const rehoboamVariant = paletteVariantShell(
  (uv, time, _hue_shift, _pulse, energy, bass, mid, high) => {
    ...
    const breathe = 0.015 * std.sin(time * 0.55);
    const ring_r = 0.52 + breathe + bass * 0.055;
    const dist = std.abs(r - ring_r);
    ...
    const ring_dir = vec2f(std.cos(angle), std.sin(angle)) * 0.75;
    const rim_noise = fbm(ring_dir + vec2f(0.0, time * 0.035) + p * 1.8);
    const broken = std.smoothstep(0.22 - high * 0.08, 0.82, rim_noise);

    // Crisp main ring: tight band with a thin soft edge, still textured by `broken`.
    const band = crispRing(r, ring_r, 0.012 + bass * 0.012, 0.018) * broken;
    ...
    const ring_gap = 0.13;
    const ring_inner = crispRing(r, ring_r - ring_gap, 0.008, 0.014);
    const ring_outer = crispRing(r, ring_r + ring_gap, 0.008, 0.014);
    const rings = ring_inner + ring_outer;

    // Keep animated fill inside the inner ring; fade out before the band.
    const inner_edge = ring_r - ring_gap;
    const inner_mask = 1.0 - std.smoothstep(inner_edge - 0.035, inner_edge + 0.008, r);

    const halo = std.exp(-dist * 9.5) * (0.12 + 0.08 * bass);

    // Audio-reactive drive only — no beat pulse or time-based sine breathing.
    const audio_drive = bass * 0.5 + mid * 0.3 + high * 0.2;
    const inner = std.exp(-r * r * 3.8) * (0.06 + 0.12 * energy * (0.35 + 0.65 * audio_drive));

    // Slow envelope-driven core motion — no beat-frequency sine churn.
    const spoke_count = 12.0 + std.floor(bass * 10.0);
    const spoke_phase = std.fract((angle / TAU) * spoke_count + mid * 0.45 + bass * 0.35);
    const spokes =
      (1.0 - std.smoothstep(0.0, 0.055 + high * 0.035, std.abs(spoke_phase - 0.5) * 2.0)) *
      std.exp(-r * r * (4.5 - bass * 1.2)) *
      (0.12 + 0.22 * audio_drive);

    const drift_uv = p * (2.6 + bass * 0.8) + vec2f(mid * 0.25, high * 0.2);
    const inner_texture =
      fbm(drift_uv + vec2f(std.sin(bass * 3.0), std.cos(mid * 2.5)) * 0.2) *
      std.exp(-r * r * 5.0) *
      (0.15 + 0.25 * energy);

    const core = std.exp(-r * r * (11.0 - audio_drive * 2.8)) * (0.08 + 0.24 * audio_drive);

    const inner_anim = (inner + spokes + inner_texture + core) * inner_mask;
    const sweep = std.pow(0.5 + 0.5 * std.cos(angle - time * (0.22 + mid * 0.5)), 3.0);
    const sweep_inner = sweep * inner_mask * (0.35 + 0.35 * energy);
    ...
    const hue_phase = std.sin(angle) * 0.06 + r * 0.38 + time * 0.012;
    ...
    const color =
      base * (halo + inner_anim * (0.7 + 0.3 * sweep_inner)) +
    ...
    const alpha = std.clamp(
      (halo * 0.42 + band * 0.78 + rings * 0.6 + inner_anim * 0.55 + core * inner_mask * 0.38) *
        enabled,
      0.0,
      0.9,
    );
