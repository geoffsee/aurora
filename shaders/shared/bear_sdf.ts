...
export const bear_rot_y = tgpu.fn(
  [vec3f, f32],
  vec3f,
)((p, a) => {
  'use gpu';
...
export const bear_smin = tgpu.fn(
  [f32, f32, f32],
  f32,
)((a, b, k) => {
  'use gpu';
...
export const bear_ellipsoid = tgpu.fn(
  [vec3f, vec3f, vec3f],
  f32,
)((p, center, radius) => {
  'use gpu';
...
export const bear_model_sdf = tgpu.fn(
  [vec3f],
  f32,
)((p) => {
  'use gpu';
  ...
  d = bear_smin(d, bear_ellipsoid(p, vec3f(-0.26, 0.86, 0.0), vec3f(0.17, 0.17, 0.14)), 0.1);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(0.26, 0.86, 0.0), vec3f(0.17, 0.17, 0.14)), 0.1);
  ...
  d = bear_smin(d, bear_ellipsoid(p, vec3f(0.46, -0.13, 0.03), vec3f(0.15, 0.42, 0.16)), 0.12);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(-0.5, -0.46, 0.12), vec3f(0.15, 0.13, 0.15)), 0.08);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(0.5, -0.46, 0.12), vec3f(0.15, 0.13, 0.15)), 0.08);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(-0.22, -0.83, 0.02), vec3f(0.19, 0.3, 0.18)), 0.13);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(0.22, -0.83, 0.02), vec3f(0.19, 0.3, 0.18)), 0.13);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(-0.22, -1.05, 0.18), vec3f(0.24, 0.13, 0.2)), 0.1);
  d = bear_smin(d, bear_ellipsoid(p, vec3f(0.22, -1.05, 0.18), vec3f(0.24, 0.13, 0.2)), 0.1);
  return d;
});

export const bear_normal = tgpu.fn(
  [vec3f],
  vec3f,
)((p) => {
  'use gpu';
...
export const bear_line_distance = tgpu.fn(
  [f32],
  f32,
)((x) => {
  'use gpu';
...
export const bear_tri_wire_2d = tgpu.fn(
  [vec2f, f32, f32, f32],
  f32,
)((uv, freq, thickness, softness) => {
  'use gpu';
  const s = vec3f(uv.x, uv.x * 0.5 + uv.y * 0.8660254, -uv.x * 0.5 + uv.y * 0.8660254).mul(freq);
  const d = std.min(
...
export const bear_surface_mesh = tgpu.fn(
  [vec3f, vec3f, f32, f32],
  f32,
)((p, n, pulse, high) => {
  'use gpu';
  ...
  const xy = bear_tri_wire_2d(
    vec2f(p.x, p.y).add(vec2f(0.03 * std.sin(p.z * 2.1), 0.0)),
    freq,
    thin,
    soft,
  );
  const yz = bear_tri_wire_2d(
    vec2f(p.y, p.z).add(vec2f(0.0, 0.025 * std.sin(p.x * 2.6))),
    freq * 0.95,
    thin,
    soft,
  );
  const xz = bear_tri_wire_2d(
    vec2f(p.x, p.z).add(vec2f(0.02 * std.sin(p.y * 2.9), 0.0)),
    freq * 1.05,
    thin,
    soft,
  );
  ...
  const major = std.clamp(
    major_xy * weights.z + major_yz * weights.x + major_xz * weights.y,
    0.0,
    1.0,
  );
  ...
  const silhouette = std.pow(
    1.0 -
      std.clamp(
        std.abs(std.dot(n, std.normalize(p.mul(-1.0).add(vec3f(0.0, 0.0, 3.0))))),
        0.0,
        1.0,
      ),
    4.0,
