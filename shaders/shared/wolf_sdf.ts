...
export const wolf_ellipse_2d = tgpu.fn(
  [vec2f, vec2f, vec2f],
  f32,
)((p, center, radius) => {
  'use gpu';
...
export const wolf_capsule_2d = tgpu.fn(
  [vec2f, vec2f, vec2f, f32],
  f32,
)((p, a, b, r) => {
  'use gpu';
  const pa = p.sub(a);
...
export const wolf_triangle_2d = tgpu.fn(
  [vec2f, vec2f, vec2f, f32],
  f32,
)((p, base, tip, width) => {
  'use gpu';
...
export const wolf_leg_2d_sdf = tgpu.fn(
  [vec2f, vec2f, f32, f32],
  f32,
)((p, hip, phase, stride) => {
  'use gpu';
  ...
  const paw = hip.add(vec2f(-0.21 * swing * stride, -0.76 + lift * 0.1));
