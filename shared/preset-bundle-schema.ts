import { type AudioEmaAlphas, DEFAULT_AUDIO_EMA_ALPHAS } from '../bridge/audio-ema.ts';
import { type AudioCurveShape, isAudioCurveShape } from './osc-validation.ts';
...
export function normalizeBandCurves(raw: Record<string, unknown> | null | undefined): BandCurves {
  const bc = raw ?? {};
...
export function migratePresetBundle(raw: unknown): PresetBundle | null {
  ...
  const state =
    r.state !== null && typeof r.state === 'object' ? (r.state as Record<string, unknown>) : {};
