...
export function generateDemoAudioFrame(bpm: number, nowSeconds: number): DemoAudioFrame {
  const tempo = Math.max(40, Math.min(240, bpm));
  const beat = ((nowSeconds * tempo) / 60) % 4;
  const energy =
    0.45 + Math.sin(nowSeconds * 2.1) * 0.2 + Math.max(0, Math.sin(nowSeconds * 8.0)) * 0.25;
