...
assertSubset('control', contract.controls.vstEmitted, contract.controls.bridgeAccepts);
assertSubset('trigger', contract.triggers.vstEmitted, contract.triggers.bridgeAccepts);
...
//      5..8 = tunnel/glitch/fluid/truchet, 9 = imported, 10..35 = newer
...
// v11: added Figure model catalog and motion/halo controls
...
export const CONTROL_STATE_SCHEMA_VERSION = 13;
...
export const validateControlStateVersion = (state: unknown, origin: string): boolean => {
  const version =
...
export const VST_CONTROL_NAMES: ReadonlySet<string> = new Set(contract.controls.bridgeAccepts);

export const VST_TRIGGER_NAMES: ReadonlySet<string> = new Set(contract.triggers.bridgeAccepts);
...
export const validateLiveOscMsg = (msg: OscMsg, origin: string): boolean => {
  if (!msg.address.startsWith('/live/')) {
    console.error(`[OSC] dropping unrecognised address "${msg.address}" from ${origin}`);
    return false;
  }
  return true;
};
...
export const validateVstOscMsg = (
  ...
  if (address.startsWith(VST_CONTROL_PREFIX)) {
    ...
    if (!VST_CONTROL_NAMES.has(name)) {
      console.error(`[VST OSC] dropping unrecognised address "${address}" from ${origin}`);
      return false;
    }
    if (args.length !== 1) {
      console.error(
