...
export function loadTriggerBindings(): TriggerBinding[] {
  try {
    return JSON.parse(localStorage.getItem(AUTOMATION_TRIGGERS_KEY) || '[]') as TriggerBinding[];
  } catch {
    return [];
  }
}
...
export function resolveTriggerAction(
  ...
  if (action === 'play' || action === 'play-loop' || action === 'stop') {
    return action;
  }
  return null;
}

export function findMidiNoteTrigger(bindings: TriggerBinding[], note: number, channel: number) {
  return bindings.find(
    (b) => b.type === 'midi-note' && b.note === note && (b.channel === 0 || b.channel === channel),
  );
}
