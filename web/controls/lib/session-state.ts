...
export function mergeSessionControlState(
  ...
  return {
    ...
    figureAssetPath: normalizeRemoteModelAssetPath(patch.figureAssetPath, defaults.figureAssetPath),
...
export function saveSessionState(state: ControlState): void {
  try {
    localStorage.setItem(SESSION_STATE_KEY, JSON.stringify(toPersistedControlState(state)));
  } catch {
