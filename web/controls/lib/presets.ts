...
export { migratePresetBundle, PRESET_BUNDLE_SCHEMA_VERSION, type PresetBundle };
...
export function defaultPendingCurves(): Record<InterpolatedKey, CurveMode> {
  return Object.fromEntries(INTERPOLATED_KEYS.map((k) => [k, 'snap' as CurveMode])) as Record<
    InterpolatedKey,
    CurveMode
  >;
}
...
export function loadPresets(): Record<string, unknown> {
  try {
    return JSON.parse(localStorage.getItem(PRESETS_KEY) || '{}') as Record<string, unknown>;
  } catch {
    return {};
  }
}
...
export function preparePresetTarget(presetState: Record<string, unknown>): Partial<ControlState> {
...
export function finalizeInterpolatedState(state: ControlState, lerped: Partial<ControlState>) {
