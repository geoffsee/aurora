...
export function applyTrackData(args: unknown[], state: ControlState, osc: OscMeters) {
  ...
  const _dtMs = Math.max(16, Math.min(250, now - (osc.lastEnvelopeAt || now)));
  ...
  osc.high = Math.max(mappedTrack(mapping.highTrack, 0), Math.min(1, transient * 2.2));
...
export function applyBrowserAudio(frame: Record<string, unknown>, osc: OscMeters) {
...
export function applyDemo(frame: Record<string, unknown>, osc: OscMeters) {
  const now = performance.now();
  osc.lastFrameAt = now;
