...
function countMidiInputs(access: MIDIAccess | null): number {
  if (!access) return 0;
  let count = 0;
  for (const _ of access.inputs.values()) {
    count += 1;
  }
  return count;
}
...
export function ControlsProvider({ children }: { children: ReactNode }) {
  ...
  const [pendingCurves, setPendingCurves] = useState(() => defaultPendingCurves());
  const [activePresetIndex, setActivePresetIndex] = useState<number | null>(null);
  const [presetRevision, setPresetRevision] = useState(0);
  const [midiBindings, setMidiBindings] = useState<MidiCcBinding[]>(() => loadMidiBindings());
  ...
  const recallPreset = useCallback(
    (n: number) => {
      ...
      const tick = () => {
        ...
        const lerped = interpolatePresetState(from, to, curves, t, transitionDurationMs);
        setState((prev) => {
          const next = { ...prev };
      ...
      if (transitionDurationMs <= 0) {
        const lerped = interpolatePresetState(from, to, curves, 1, transitionDurationMs);
        setState((prev) => {
          const next = { ...prev };
          finalizeInterpolatedState(next, lerped);
          stateRef.current = next;
  ...
  const onMidiMessage = useCallback(
    (event: MIDIMessageEvent) => {
      ...
      if (msgType === 0xb0) {
        ...
        const trigger = findMidiCcTrigger(triggerBindings, data1, channel, data2);
  ...
  const startMicCapture = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        addBanner(`Live mic capture failed: ${e?.name || 'Error'} — ${e?.message || err}`, 'error');
        return;
      }
      const AudioCtx =
        window.AudioContext ||
  ...
  const applyFrame = useCallback(
    (frame: { address?: string; args?: unknown[]; error?: unknown; id?: number }) => {
      ...
      if (frame?.address === '/aurora/audio/features' && args[0] && typeof args[0] === 'object') {
        setOsc((prev) => {
          const next = { ...prev };
  ...
  useEffect(() => {
    ...
    const pingTimer = staticPreview.current ? null : setInterval(sendPing, 1000);
    return () => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const demo = generateDemoAudioFrame(stateRef.current.bpm, Date.now() / 1000);
