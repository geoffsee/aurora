...
export function RehearsalPanel() {
  const { state, isRecording, recording, startRecording, playReplay, stopReplay, updateState } =
    useControls();
  ...
  const recordingState = state.replaying ? 'replaying' : isRecording ? 'recording' : 'ready';
...
export function AudioControlPanel() {
  ...
  const badgeLabel = state.audioControlMode ? (micActive ? 'Mic live' : 'On') : 'Off';
  const badgeState =
    state.audioControlMode && micActive ? 'live' : state.audioControlMode ? 'info' : 'idle';
    ...
    <Panel area="audc" aria-label="Audio Control">
      ...
      <Flex gap={2} wrap="wrap">
        <Button
          ...
          onClick={() => updateState({ audioControlMode: !state.audioControlMode })}
        >
          Audio Control
        </Button>
        <Button
