import { Box, Button, Field, Flex, Grid, Input, NativeSelect, Text } from '@chakra-ui/react';
...
import { MAPPABLE_PARAMS, type MappableParam, PARAM_META } from '../lib/param-meta.ts';
  ...
  return (
    <Panel area="midi" aria-label="MIDI CC Mapping">
      ...
      <Box aria-label="CC bindings list" mb={3}>
        {!midiBindings.length ? (
          <Text color="whiteAlpha.700">No bindings — use Add Binding to create one.</Text>
        ) : (
          midiBindings.map((b, i) => {
            const chStr = b.channel === 0 ? 'any' : `ch${b.channel}`;
            return (
              <Flex
                key={`${b.cc}-${b.channel}-${b.param}-${b.paramMin}-${b.paramMax}`}
                align="center"
              ...
              >
                <Text fontSize="sm">
                  CC{b.cc} ({chStr}) → {MIDI_CC_PARAM_LABELS[b.param] || b.param} [{b.paramMin}–
                  {b.paramMax}]
                </Text>
                <Button
                  size="sm"
...
export function TriggersPanel() {
  const { state, updateState, triggerBindings, addTriggerBinding, removeTriggerBinding } =
    useControls();
    ...
    <Panel area="trig" aria-label="Automation Triggers">
      <SectionTitle
        ...
        badge={
          <StatusPill state={state.audioTransientAutomation ? 'live' : 'info'}>
            {state.audioTransientAutomation
              ? 'Audio armed'
              : `${triggerBindings.length} trigger${triggerBindings.length !== 1 ? 's' : ''}`}
          </StatusPill>
        }
      />
      ...
      <Box aria-label="Trigger bindings list" mb={3}>
        {!triggerBindings.length ? (
          <Text color="whiteAlpha.700">No triggers — use Add Trigger to create one.</Text>
        ) : (
          triggerBindings.map((b, i) => {
            ...
            if (b.type === 'midi-note') {
              ...
              key = `note-${b.channel}-${b.note}-${b.action}`;
            } else if (b.type === 'midi-cc') {
              const chStr = b.channel === 0 ? 'any' : `ch${b.channel}`;
              label = `CC${b.cc} (${chStr}) ≥ ${b.threshold} → ${b.action}`;
              key = `cc-${b.channel}-${b.cc}-${b.threshold}-${b.action}`;
            } else {
              label = `OSC ${b.address} → ${b.action}`;
              key = `osc-${b.address}-${b.action}`;
            }
            return (
              <Flex
                key={key}
        ...
        <Grid templateColumns="repeat(auto-fit, minmax(160px, 1fr))" gap={3} mt={3}>
          <Field.Root>
            ...
            <NativeSelect.Root>
              <NativeSelect.Field
                value={type}
                onChange={(e) => setType(e.target.value as TriggerBinding['type'])}
              >
          ...
          {type === 'osc' ? (
            <Field.Root gridColumn="1 / -1">
              ...
              <Input value={oscAddress} onChange={(e) => setOscAddress(e.target.value)} />
            </Field.Root>
          ) : null}
          <Field.Root>
            <Field.Label>Action</Field.Label>
            <NativeSelect.Root>
              <NativeSelect.Field value={action} onChange={(e) => setAction(e.target.value)}>
