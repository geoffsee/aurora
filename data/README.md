...
Each successful catalog scan produces a snapshot with a monotonic **epoch**. The epoch advances only when the merged content hash changes (no-op rescans do not churn). List APIs expose epoch for cache invalidation (PR3+).
