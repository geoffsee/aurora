...
const layer = (name: string, state: PresetLayer['state'], weight: number): PresetLayer => ({
  name,
  state,
  weight,
});
...
describe('composeLayers', () => {
  ...
  test('a key the base omits is established by the first layer that provides it', () => {
    const out = composeLayers({}, [layer('a', { depth: 0.6 }, 0.5), layer('b', { depth: 1 }, 0.5)]);
...
describe('stack operations are pure', () => {
  const stack = [layer('a', { intensity: 1 }, 0.5), layer('b', { palette: 1 }, 1)];
...
describe('validatePresetLayerOscMsg', () => {
  ...
  test('add rejects unknown cues, missing/extra args, non-string names', () => {
    ...
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_ADD_ADDRESS, args: [] }, 'test', cues),
    ).toBe(false);
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_ADD_ADDRESS, args: [1] }, 'test', cues),
    ).toBe(false);
    expect(
      validatePresetLayerOscMsg(
  ...
  test('weight requires two numeric args', () => {
    ...
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_WEIGHT_ADDRESS, args: [0] }, 'test', cues),
    ).toBe(false);
    expect(
      validatePresetLayerOscMsg(
  ...
  test('remove requires one numeric index', () => {
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_REMOVE_ADDRESS, args: [1] }, 'test', cues),
    ).toBe(true);
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_REMOVE_ADDRESS, args: [] }, 'test', cues),
    ).toBe(false);
  });

  test('move requires two numeric args; clear takes none', () => {
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_MOVE_ADDRESS, args: [0, 1] }, 'test', cues),
    ).toBe(true);
    expect(
      validatePresetLayerOscMsg({ address: PRESET_LAYER_CLEAR_ADDRESS, args: [] }, 'test', cues),
    ).toBe(true);
  });

  test('unrecognised layer command is rejected', () => {
    expect(
      validatePresetLayerOscMsg({ address: '/aurora/preset/layer/bogus', args: [] }, 'test', cues),
    ).toBe(false);
  });
});
