...
describe('validateLiveOscMsg', () => {
  test('accepts /live/ address', () => {
    expect(validateLiveOscMsg({ address: '/live/song/get/tempo' }, 'test')).toBe(true);
  });

  test('rejects address without /live/ prefix', () => {
    expect(validateLiveOscMsg({ address: '/aurora/something' }, 'test')).toBe(false);
  });
...
describe('validateVstOscMsg control branch', () => {
  ...
  test('accepts known control with plain number arg', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/control/bpm', args: [120] }, 'test', CUE_NAMES),
    ).toBe(true);
  });
  ...
  test('rejects control with zero args', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/control/crossfade', args: [] }, 'test', CUE_NAMES),
    ).toBe(false);
  });
...
describe('validateVstOscMsg trigger branch', () => {
  ...
  test('rejects trigger with wrong arg count', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/trigger/flash', args: [] }, 'test', CUE_NAMES),
    ).toBe(false);
  });
...
describe('validateVstOscMsg cue branch', () => {
  test('accepts known cue with float arg', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/cue/drop', args: [floatArg] }, 'test', CUE_NAMES),
    ).toBe(true);
  });
  ...
  test('rejects cue with zero args', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/cue/drop', args: [] }, 'test', CUE_NAMES),
    ).toBe(false);
  });

  test('rejects cue with non-numeric arg', () => {
    expect(
      validateVstOscMsg({ address: '/aurora/vst/cue/drop', args: [stringArg] }, 'test', CUE_NAMES),
    ).toBe(false);
  });
});
...
describe('validatePresetOscMsg', () => {
  test('accepts valid recall address for slot 1', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/recall/1' }, 'test')).toBe(true);
  });

  test('accepts valid recall address for slot 6', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/recall/6' }, 'test')).toBe(true);
  });

  test('accepts valid save address', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/save/3' }, 'test')).toBe(true);
  });

  test('rejects slot 0 (below min)', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/recall/0' }, 'test')).toBe(false);
  });

  test('rejects slot 7 (above max)', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/save/7' }, 'test')).toBe(false);
  });

  test('rejects non-integer slot', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/recall/foo' }, 'test')).toBe(false);
  });

  test('rejects fractional slot', () => {
    expect(validatePresetOscMsg({ address: '/aurora/preset/recall/1.5' }, 'test')).toBe(false);
  });

  test('rejects unrelated aurora address', () => {
    expect(validatePresetOscMsg({ address: '/aurora/control/state' }, 'test')).toBe(false);
  });
      ...
      validatePresetMorphOscMsg(
        {
          ...
          args: [{ type: 's', value: 'warmup' }, { type: 's', value: 'drop' }, floatArg],
        },
        'test',
        CUE_NAMES,
      ),
...
describe('validateControlStateVersion', () => {
  ...
  test('rejects state with schemaVersion set to zero', () => {
    expect(validateControlStateVersion({ schemaVersion: 0 }, 'test')).toBe(false);
  });
});
...
describe('validateVstOscMsg unrecognised prefix', () => {
  test('rejects completely unknown address', () => {
    expect(
      validateVstOscMsg({ address: '/unknown/path', args: [floatArg] }, 'test', CUE_NAMES),
    ).toBe(false);
  });

  test('rejects /live/ address', () => {
    expect(
      validateVstOscMsg({ address: '/live/song/get/tempo', args: [] }, 'test', CUE_NAMES),
    ).toBe(false);
  });
});
