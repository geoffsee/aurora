...
const clampInt = (value: number, lo: number, hi: number, fallback: number): number => {
  ...
  for (const { name, route: raw } of cases) {
    test(`applyOutputRoute matches resolveOutputView: ${name}`, () => {
      ...
      for (const b of [base(), { ...base(), blackout: true }]) {
        expect(applyOutputRouteReplica(b, route)).toEqual(resolveOutputView(b, route));
      }
    });
  }
