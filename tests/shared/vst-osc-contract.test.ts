...
describe('VST OSC contract', () => {
  test('vstEmitted is a subset of bridgeAccepts for each kind', () => {
    const checkSubset = (emitted: readonly string[], accepted: readonly string[]) => {
  ...
  test('contract files are kept in sync at module load', async () => {
    ...
    expect(mod.VST_CONTROL_NAMES.size).toBe(contract.controls.bridgeAccepts.length);
    expect(mod.VST_TRIGGER_NAMES.size).toBe(contract.triggers.bridgeAccepts.length);
  });
});
