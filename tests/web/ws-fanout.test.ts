...
function waitForControlStateWithCrossfade(
  ...
  return new Promise((resolve, reject) => {
    const timerId = setTimeout(
      () =>
        reject(
          new Error(`No control state with crossfade≈${crossfade} received within ${timeoutMs}ms`),
        ),
      timeoutMs,
    );
...
test('controls-page state mutation is reflected on projector page within 500ms', async () => {
  const [projectorWs, controlsWs] = await Promise.all([openWebSocket(), openWebSocket()]);
...
test('preset recall command is broadcast to all connected clients', async () => {
  const [senderWs, observerWs] = await Promise.all([openWebSocket(), openWebSocket()]);
  ...
  try {
    ...
    senderWs.send(JSON.stringify({ address: '/aurora/preset/recall/3', args: [] }));
...
test('second simultaneous controls-page client receives the same state update', async () => {
  ...
  try {
    const projectorReceived = waitForControlStateWithCrossfade(projectorWs, 0.67);
