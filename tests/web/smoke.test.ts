...
function addBanner(container: Element, description: string, type?: string): Element {
...
test('addBanner caps at 10 and evicts the oldest', () => {
  ...
  expect(banners.item(0)?.querySelector('.error-banner__msg')?.textContent).toBe('Error 1');
});
...
test('applyFrame shows banner for /aurora/error frames', () => {
  ...
  expect(container.querySelector('.error-banner__msg')?.textContent).toBe('something broke');
...
test('disconnect banners are removed on reconnect', () => {
  ...
  document.querySelectorAll("[data-banner-type='disconnect']").forEach((b) => {
    b.remove();
  });
  expect(container.querySelectorAll('.error-banner').length).toBe(1);
