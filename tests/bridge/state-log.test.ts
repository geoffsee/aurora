...
test('diffObjects detects nested object changes via shallow comparison', () => {
  ...
  const diff = diffObjects(prev as Record<string, unknown>, next as Record<string, unknown>);
...
test('diffObjects returns null when nested object is unchanged', () => {
  ...
  expect(diffObjects(prev as Record<string, unknown>, next as Record<string, unknown>)).toBeNull();
});
