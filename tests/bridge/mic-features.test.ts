...
import { extractMicFeatures, micSecureContextError } from '../../bridge/mic-features.ts';
...
describe('micSecureContextError', () => {
  test('allows secure contexts and loopback hosts', () => {
    expect(micSecureContextError({ isSecureContext: true, hostname: 'vj.example' })).toBeNull();
    expect(micSecureContextError({ isSecureContext: false, hostname: 'localhost' })).toBeNull();
    expect(micSecureContextError({ isSecureContext: false, hostname: '127.0.0.1' })).toBeNull();
  });
...
describe('extractMicFeatures', () => {
  test('always returns all five bands clamped to 0..1', () => {
    expect(inRange01(extractMicFeatures(spectrumInBand(0, 9999), opts))).toBe(true);
    ...
    expect(extractMicFeatures(spectrumInBand(0, 9999), { ...opts, sampleRate: 0 })).toEqual({
      energy: 0,
      bass: 0,
      mid: 0,
      high: 0,
...
const inlineClamp = (value: number, min: number, max: number, fallback = min) => {
...
function inlineSecureContextError(isSecureContext: boolean, hostname: string): string | null {
...
function inlineExtractMicFeatures(
  ...
  for (let i = 0; i < bins; i++) {
    ...
    if (hz >= INLINE_MIC_BANDS.bass[0] && hz < INLINE_MIC_BANDS.bass[1]) {
      ...
      midSum += norm;
      midCount++;
    } else if (hz >= INLINE_MIC_BANDS.high[0] && hz < INLINE_MIC_BANDS.high[1]) {
      highSum += norm;
      highCount++;
  ...
  const highValue =
    highMean * (1 - INLINE_MIC_HIGH_PEAK_BIAS) + highPeak * INLINE_MIC_HIGH_PEAK_BIAS;
  return {
    energy: inlineClamp01(energySum / bins),
    bass: bassCount ? inlineClamp01((bassSum / bassCount) * INLINE_MIC_BAND_GAIN.bass) : 0,
    mid: midCount ? inlineClamp01((midSum / midCount) * INLINE_MIC_BAND_GAIN.mid) : 0,
...
describe('controls.html inline parity', () => {
  test('extractMicFeatures matches the module across representative spectra', () => {
    ...
    for (const spectrum of spectra) {
      expect(inlineExtractMicFeatures(spectrum, SAMPLE_RATE, FFT_SIZE)).toEqual(
        extractMicFeatures(spectrum, opts),
      );
    }
