...
describe('deriveLinkFrame', () => {
  test('returns tempo and wrapped beat phase for a valid snapshot', () => {
    const frame = deriveLinkFrame({ beat: 9.5, phase: 1.5, bpm: 128, numPeers: 2 }, 4);
  ...
  test('works with zero peers (graceful solo timeline)', () => {
    const frame = deriveLinkFrame({ beat: 0, phase: 0, bpm: 120, numPeers: 0 }, 4);
    expect(frame).not.toBeNull();
    expect(frame?.tempo).toBe(120);
  });

  test('accepts the tempo range boundaries', () => {
    expect(deriveLinkFrame({ beat: 0, phase: 0, bpm: LINK_TEMPO_MIN, numPeers: 0 }, 4)?.tempo).toBe(
      LINK_TEMPO_MIN,
    );
    expect(deriveLinkFrame({ beat: 0, phase: 0, bpm: LINK_TEMPO_MAX, numPeers: 0 }, 4)?.tempo).toBe(
      LINK_TEMPO_MAX,
    );
  });

  test('rejects tempo below the minimum', () => {
    expect(deriveLinkFrame({ beat: 0, phase: 0, bpm: 39, numPeers: 0 }, 4)).toBeNull();
  });

  test('rejects tempo above the maximum', () => {
    expect(deriveLinkFrame({ beat: 0, phase: 0, bpm: 241, numPeers: 0 }, 4)).toBeNull();
  });

  test('rejects non-finite tempo', () => {
    expect(deriveLinkFrame({ beat: 0, phase: 0, bpm: Number.NaN, numPeers: 0 }, 4)).toBeNull();
  });

  test('wraps the beat phase into the quantum', () => {
    const frame = deriveLinkFrame({ beat: 0, phase: 4.25, bpm: 120, numPeers: 1 }, 4);
