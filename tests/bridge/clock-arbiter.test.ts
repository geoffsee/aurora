...
describe('selectTempoSource priority', () => {
  test('Link wins when active, even alongside MIDI clock', () => {
    expect(selectTempoSource({ linkActive: true, midiActive: true })).toBe('link');
    expect(selectTempoSource({ linkActive: true, midiActive: false })).toBe('link');
  });

  test('MIDI clock wins when Link is absent', () => {
    expect(selectTempoSource({ linkActive: false, midiActive: true })).toBe('midi');
  });

  test('internal is the floor when no external source is active', () => {
    expect(selectTempoSource({ linkActive: false, midiActive: false })).toBe('internal');
  });
...
describe('tempo mirror does not flap when sources disagree', () => {
  ...
  function runMirror(
    ...
    for (let i = 0; i < ticks; i++) {
      ...
      if (activity.linkActive && selectTempoSource(activity) === 'link') mirror.push(LINK_BPM);
      if (activity.midiActive && selectTempoSource(activity) === 'midi') mirror.push(MIDI_BPM);
