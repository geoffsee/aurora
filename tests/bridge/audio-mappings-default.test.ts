...
import { makeAudioControlRouter, parseAudioMappings } from '../../bridge/audio-control-router.ts';
import {
  type AudioFeatures,
...
import audioMappingsRaw from '../../bridge/audio-mappings.json' with { type: 'json' };
...
const demoRawFeatures = (nowSec: number): AudioFeatures => {
  ...
  const energy = 0.45 + Math.sin(nowSec * 2.1) * 0.2 + Math.max(0, Math.sin(nowSec * 8.0)) * 0.25;
  return {
...
const countFlashesOverDemoWindow = (pulseAlpha: number, windowMs: number) => {
  ...
  const flashMapping = shipped.filter((m) => m.source === 'pulse' && m.target === 'flashVersion');
  ...
  for (let t = 0; t < windowMs; t += DEMO_LOOP_MS) {
    ...
    const smoothed = stepAudioEma(ema, raw, alphas, DEFAULT_AUDIO_EMA_RELEASE_ALPHAS);
...
describe('shipped default audio mappings (#199)', () => {
  test('a pulse→flashVersion threshold mapping is shipped', () => {
    const shipped = parseAudioMappings(audioMappingsRaw);
    const flash = shipped.find((m) => m.source === 'pulse' && m.target === 'flashVersion');
  ...
  test.each([0.28, 0.85])(
    'default flash fires under the demo pulse with pulse alpha %s',
    (pulseAlpha) => {
