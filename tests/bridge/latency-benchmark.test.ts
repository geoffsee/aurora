...
test('E2E pipeline P95 latency is under 100ms', async () => {
  ...
  const [sender, receiver] = await Promise.all([openWebSocket(), openWebSocket()]);
...
test('ping handler returns pong with matching id', async () => {
  ...
  try {
    const pong = await new Promise<{ address: string; id: number }>((resolve, reject) => {
      const timerId = setTimeout(() => reject(new Error('pong not received within 1000ms')), 1000);
