...
import { MIDI_CC_INTEGER_PARAMS, MIDI_CC_PARAM_LABELS } from '../../web/controls/lib/constants.ts';
import { MAPPABLE_PARAMS, PARAM_META } from '../../web/controls/lib/param-meta.ts';
...
describe('layerWeightFields', () => {
  ...
  test('clamps out-of-range and non-finite weights into the unit interval', () => {
    const fields = layerWeightFields([layer('a', {}, 2), layer('b', {}, Number.NaN)]);
...
describe('changedLayerWeightIndices', () => {
  ...
  test('no layer-weight change yields an empty list', () => {
    expect(changedLayerWeightIndices({ crossfade: 0 }, { crossfade: 1 })).toEqual([]);
  });
});
...
describe('applyLayerWeightControl (external weight edit → recomposite)', () => {
  ...
  test('dropping the slot to 0 recomposes back to the floor', () => {
    const h = makeHarness({ intensity: 0.2 });
    h.controller.add({ name: 'a', state: { intensity: 1 }, weight: 1 });
    applyLayerWeightControl(h.controller, { ...h.live() }, { ...h.live(), layerWeight0: 0 });
  ...
  test('editing a slot with no live layer is a no-op reset to 0', () => {
    ...
    applyLayerWeightControl(h.controller, { ...h.live() }, { ...h.live(), layerWeight4: 0.8 });
...
describe('automation target', () => {
  ...
  test('a layer-weight change is recorded and survives the diff filter', () => {
    expect(filterAutomationDiff({ layerWeight2: 0.6, strobeLockout: true })).toEqual({
      layerWeight2: 0.6,
    });
...
describe('schema migration', () => {
  test('v9 state gains layer-weight slots and Figure controls by v13', () => {
