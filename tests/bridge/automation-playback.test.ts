...
function dataFrames(applied: Record<string, unknown>[]): Record<string, unknown>[] {
    ...
    const entries = [
      {
        ...
        diff: {
          ...
          deckAMode: 3,
          deckBMode: 5,
          deckAPresetSlug: 'wash',
          deckBPresetSlug: 'mirror',
          activeShader: 7,
          deckAGpuShader: 20,
          deckBGpuShader: 26,
...
describe('makeAutomationPlayer', () => {
  ...
  test('play() emits replaying:true and stop emits replaying:false', () => {
    ...
    const rec = buildRecording([{ ts: 0, diff: { intensity: 0.5 } }], CONTROL_STATE_SCHEMA_VERSION);
