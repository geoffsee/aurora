...
const tileGrid: Shader = {
  ...
  mainImage: (fragX, fragY, ctx) => {
    const cols = 8;
    const rows = Math.max(1, Math.round((cols * ctx.iResolutionY) / ctx.iResolutionX));
...
export const SHADERS: readonly Shader[] = [paletteGradient, beamBurst, tileGrid, shadertoyPlasma];
...
export const encodePng = (fb: Framebuffer): Uint8Array => {
  ...
  for (let row = 0; row < fb.height; row++) {
    raw[row * (stride + 1)] = 0;
    raw.set(fb.rgb.subarray(row * stride, (row + 1) * stride), row * (stride + 1) + 1);
  }
...
export const decodePng = (bytes: Uint8Array): Framebuffer => {
  ...
  for (let row = 0; row < height; row++) {
    ...
    rgb.set(raw.subarray(row * (stride + 1) + 1, row * (stride + 1) + 1 + stride), row * stride);
  }
  return { width, height, rgb };
};
