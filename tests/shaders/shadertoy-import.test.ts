...
describe('extractShadertoyId', () => {
  ...
  test('extracts ID from full Shadertoy URL', () => {
    expect(extractShadertoyId('https://www.shadertoy.com/view/ftGGWm')).toBe('ftGGWm');
    expect(extractShadertoyId('http://shadertoy.com/view/XdXGzr')).toBe('XdXGzr');
    expect(extractShadertoyId('shadertoy.com/view/Wd3Bz4?foo=bar')).toBe('Wd3Bz4');
  });
  ...
  test('rejects IDs longer than 16 chars', () => {
    ...
    expect(extractShadertoyId(`https://shadertoy.com/view/${'a'.repeat(17)}`)).toBeNull();
  });
  ...
  test('rejects empty / non-matching strings', () => {
    ...
    expect(extractShadertoyId('https://example.com/view/abc123')).toBe('abc123');
  });

  test('rejects path traversal in URL form', () => {
    expect(extractShadertoyId('https://shadertoy.com/view/../../../etc/passwd')).toBeNull();
  });
});
...
describe('adaptNagaWgslForBevy', () => {
  ...
  test("rewrites naga's main entry point into a Bevy fragment fn", () => {
    ...
    expect(adapted).toContain('#import bevy_sprite::mesh2d_vertex_output::VertexOutput');
