...
test("v1 state is upgraded through v2, v3, v4, v5 with activeShader=0, bandCurves, emaAlphas, and morphWeight added", () => {
	...
	expect(result.morphWeight).toBe(0);
});

test("v2 state is upgraded through v3, v4, then v5", () => {
	const result = migrateControlState({
		schemaVersion: 2,
		activeShader: 1,
	}) as Record<string, unknown>;
...
test("v3 state is upgraded to v5 with emaAlphas and morphWeight added", () => {
	const input = {
		schemaVersion: 3,
		activeShader: 1,
		bandCurves: {
...
test("v4 state is upgraded to v5 with morphWeight added", () => {
	const input = {
		schemaVersion: 4,
		activeShader: 1,
		bandCurves: {
...
test("v5 state passes through unchanged", () => {
	const input = {
		schemaVersion: 5,
		activeShader: 1,
		bandCurves: {
